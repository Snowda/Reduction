...
fn demonstrate_health(health_tx: &watch::Sender<HealthState>) {
    ...
    let backends: Vec<BackendConfig> = vec![
        BackendConfig::new("node-1", "10.0.0.1:8080".parse().unwrap(), 1.0, TransportKind::Tcp).unwrap(),
        BackendConfig::new("node-2", "10.0.0.2:8080".parse().unwrap(), 1.0, TransportKind::Tcp).unwrap(),
    ];
